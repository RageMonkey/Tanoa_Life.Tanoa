...
if ( life_spectator_admin_lvl > __GETC__(life_adminlevel) ) exitWith { hint "Insufficient Admin Level";}; //Check Admin Level
if (isNil "godmode") then {	godmode = 0;};
if ( godmode != 1 && life_ESP_admin_lvl_Always > __GETC__(life_adminlevel)) exitWith { hint "God Mode must be activated to use Spectate";};
[[getPlayerUID player,name player, "Spectate activated",position player,""],"TON_fnc_logInReport",false,false] spawn life_fnc_MP;
...
spectate_events = compile preprocessFileLineNumbers "dialog\spectator\specta_events.sqf";
RefreshPlayerList = compile preprocessFileLineNumbers "dialog\spectator\RefreshPlayerList.sqf";
FreeLookMovementHandler = compile preprocessFileLineNumbers "dialog\spectator\FreeLookMovementHandler.sqf";
CameraMenuHandler = compile preprocessFileLineNumbers "dialog\spectator\CameraMenuHandler.sqf";
PlayerMenuHandler = compile preprocessFileLineNumbers "dialog\spectator\PlayerMenuHandler.sqf";
KEGsShowCombatMode = compile preprocessFileLineNumbers "dialog\spectator\specta_combatmode.sqf";
KEGsShowUnitLocator = compile preprocessFileLineNumbers "dialog\spectator\specta_locator.sqf";
