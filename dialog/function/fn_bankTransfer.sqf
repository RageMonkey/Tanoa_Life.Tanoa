...
if(_val > life_RoT_atm) exitWith {hint localize "STR_ATM_NotEnough"};
_tax = [_val] call life_fnc_taxRate;
if((_val + _tax) > life_RoT_atm) exitWith {hint format[localize "STR_ATM_SentMoneyFail",_val,_tax]};

life_RoT_atm = life_RoT_atm - (_val + _tax);
