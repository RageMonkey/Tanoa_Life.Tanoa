...
if(_curTarget isKindOf "House_F") exitWith {
	if((nearestObject [[12940.9,7241.42,0],"Land_Dome_Big_F"]) == _curTarget OR (nearestObject [[12940.9,7241.42,0],"Land_Research_house_V1_F"]) == _curTarget) then {
...
//if(!((player distance (getMarkerPos "police_hq_1") < 30) OR  (player distance (getMarkerPos "police_hq_2") < 30) OR (player distance (getMarkerPos "cop_spawn_3") < 30) OR (player distance (getMarkerPos "cop_spawn_5") < 30) OR (player distance (getMarkerPos "cop_spawn_cmd") < 30))) then 
//if(!((player distance (getMarkerPos "police_hq_1") < 30) OR  (player distance (getMarkerPos "police_hq_2") < 30) OR (player distance (getMarkerPos "cop_spawn_5") < 30) OR (player distance (getMarkerPos "cop_spawn_cmd") < 30))) then 

if(!((player distance (getMarkerPos "jail_1") < 40) OR (player distance (getMarkerPos "jail_2") < 40))) then 
{
