...
*/
//private["_ui","_units","_goggles","_uniform","_headgear"]; #GOGGLES
private["_ui","_units","_uniform","_headgear"];
#define iconID 78000
#define scale 0.8

//_goggles = []; #GOGGLES
_uniform = ["U_I_GhillieSuit","U_O_GhillieSuit"];
_headgear = ["H_Shemag_olive_hs","H_ShemagOpen_tan","H_ShemagOpen_khk"];
	...
	if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x getVariable "realname"}}) then {
		...
		if(count _sPos > 1 && {_distance < 15}) then {
			_text = switch (true) do {
				//case ((goggles _x) in _goggles): {format["<t color='#000000'>Masked Player</t>"];}; #GOGGLES
