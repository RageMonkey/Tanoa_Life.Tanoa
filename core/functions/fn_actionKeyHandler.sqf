...
if(isNull _curTarget) exitWith {
	if(_isWater) then {
		...
		if(!life_action_gathering) then {		//Remove "playerSide == civilian &&" so cops can gather stuff -> limited on gather function
...
if(_curTarget isKindOf "House_F" && {player distance _curTarget < 12} OR ((nearestObject [[12940.9,7241.42,0],"Land_Dome_Big_F"]) == _curTarget OR (nearestObject [[12940.9,7241.42,0],"Land_Research_house_V1_F"]) == _curTarget)) exitWith {
...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	...
	if(_isVehicle) then {
		...
		if((typeOf _curTarget) in _animalTypes) then {
			if((typeOf _curTarget) == "Turtle_F" && !alive _curTarget && ((player distance (getMarkerPos "turtle_1") < 350) OR (player distance (getMarkerPos "turtle_2") < 350) OR (player distance (getMarkerPos "turtle_3") < 350))) then {
				private["_handle"];
