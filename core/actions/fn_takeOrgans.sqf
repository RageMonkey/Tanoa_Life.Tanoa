...
private["_unit"];
_unit = cursorTarget;
if({player distance getMarkerPos (_x select 0) < _x select 1} count __GETC__(life_safe_points) > 0) exitWith {	hint "You cannot harvest organs in a Safezone" }; //Abort if Safe Zone
if(!([false,"scalpel",1] call life_fnc_handleInv)) exitWith {hint "You don't have a scalpel in your Inventory"};
if(isNull _unit) exitWith {}; //if unit is null, than NO
if((_unit getVariable ["missingOrgan",FALSE])) exitWith {hint "Victim is already missing an Organ"};//must not be missing organ already
if((player getVariable ["hasOrgan",FALSE])) exitWith {hint "Organ harvesting is tiring work, take a break!"};//thief must not have already robbed an organ within last 5 minutes
